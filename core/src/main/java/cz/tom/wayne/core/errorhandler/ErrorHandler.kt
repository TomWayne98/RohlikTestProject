 ...
 * object and set its [NetworkTask.state] to [FireProgress].
 * Then if the task succeeds, call [handleEvent] again with the task's state set to [FireSuccess],
 ...
 * which returns a live data for lifecycle-aware callers. However, the UI handled by [ErrorHandler] is non-dependant on the listeners, so if a task
 * has no listeners set, the dialogs will still be displayed. After a task ends, all listeners for it all cleared.
 *
 * UI HANDLING:
 * [ErrorHandler] handles and shows some UI automatically depending on the task state:
 * (To disable UI handling, set your task's [NetworkTask.showUi] to false)
 * If it is a [FireProgress], dismisses all previously shown error dialogs and shows a non-dismissable, non-cancelable progress dialog
 *   with progress text related to the task, but only if [NetworkTask.showUi] is set to true.
 * If it is a [FireSuccess], dismisses all previously shown progress dialogs, but only if [NetworkTask.showUi] is set to true.
